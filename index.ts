...
events.onmessage = (ev) => {
    try {
        ...
        const titleIssue:string = decipher.update(JSON.parse(ev.data).body.title, 'base64', 'utf8').trim()
        decipher.final()
        console.log(titleIssue)
        decipher = createDecipheriv('aes-256-cbc', aesKey, aesIV)
        decipher.setAutoPadding(false)
        const descIssue:string = decipher.update(JSON.parse(ev.data).body.description, 'base64', 'utf8').trim()
        decipher.final()
        console.log(descIssue)
        decipher = createDecipheriv('aes-256-cbc', aesKey, aesIV)
        decipher.setAutoPadding(false)
        const steamToken:string = decipher.update(JSON.parse(ev.data).body.steamtoken, 'base64', 'utf8').trim()
        decipher.final()
        console.log(steamToken)
        decipher = createDecipheriv('aes-256-cbc', aesKey, aesIV)
        decipher.setAutoPadding(false)
        const typeIssue:string = decipher.update(JSON.parse(ev.data).body.type, 'base64', 'utf8').trim()
        decipher.final()
        console.log(typeIssue)
        decipher = createDecipheriv('aes-256-cbc', aesKey, aesIV)
        decipher.setAutoPadding(false)
        const uuid:string = decipher.update(JSON.parse(ev.data).body.uuid, 'base64', 'utf8').trim()
        decipher.final()
        console.log(uuid)
        // uuidv4 validator
